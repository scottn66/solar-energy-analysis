...
DISCLAIMER = (
    ...
    "Shading, your actual tariff, installer quotes, and local incentives can "
    "change these numbers. Compare with a licensed contractor before you spend."
)

# Human-readable labels for the assumptions table.
...
ASSUMPTION_LABELS = {
    ...
    "electricity_price_override": ("Electricity rate used", "$/kWh", 3),
    "electricity_escalation": ("Rate escalation", "%/yr", 1),
    "nem_export_ratio": ("Export credit vs retail", "", 2),
    "self_consumption_rate": ("Self-consumption", "%", 0),
    "co2_intensity_tons_per_kwh": ("Grid CO2 intensity", "t/kWh", 4),
    "default_price_per_watt": ("Install cost used", "$/W", 2),
}


def format_assumption(key: str, val) -> tuple[str, str]:
    """Return (human label, display value) for a model assumption."""
    info = ASSUMPTION_LABELS.get(key)
    if info is None:
        return key.replace("_", " ").title(), "—" if val is None else str(val)
    name, unit, decimals = info
    if val is None:
        return name, "—"
    if isinstance(val, (int, float)):
        if "%" in unit and abs(val) <= 1:
            return name, f"{val * 100:.{decimals}f}{unit}"
        if unit.startswith("$") and "/kWh" in unit:
            return name, f"${val:.{decimals}f}/kWh"
        if unit.startswith("$") and "/W" in unit:
            return name, f"${val:.{decimals}f}/W"
        if unit.startswith("$"):
            return name, f"${val:.{decimals}f} {unit.replace('$', '').strip()}"
        if unit:
            return name, f"{val:.{decimals}f} {unit}" if isinstance(val, float) else f"{val} {unit}"
        return name, f"{val:.{decimals}f}"
    return name, str(val)
...
def generate_report(
    ...
    for key, val in a.items():
        if key.startswith("weight_"):
            continue
        label, display_val = format_assumption(key, val)
        assumptions_rows += f"""
        <tr>
    ...
    <div class="section">
        ...
        <div class="chart-card">
            <p style="font-size: 13px; color: {PALETTE['text_secondary']}; margin-bottom: 16px;">
                Every number in the 25-year cashflow is documented here and can be overridden.
...
<div class="footer">
    {DISCLAIMER}<br><br>
    Solar viability report &middot; Scott Nelson &middot;
    Data: NREL PVWatts, OpenEI URDB, EIA, Berkeley Lab Tracking the Sun
...
def _build_accordion(site_result: SiteResult, quote) -> str:
    ...
    for k, v in site_result.assumptions_used.items():
        if k.startswith("weight_"):
            continue
        name, display = format_assumption(k, v)
